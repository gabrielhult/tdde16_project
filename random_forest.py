...
def decisive_words(model, vectoriser, model_name):
    ...
    liberal_top_words = [vectoriser.get_feature_names_out()[i] for i in np.argsort(feature_importances)[-10:][::-1]]
    conservative_top_words = [vectoriser.get_feature_names_out()[i] for i in np.argsort(feature_importances)[:10]]
    ...
    # Save the top words with their importances for liberal to a text file
    ...
    # Save the top words with their importances for conservative to a text file
